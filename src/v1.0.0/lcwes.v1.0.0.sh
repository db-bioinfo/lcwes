...
#python OmicMind_wes_chr2int.v1.0.0.py ${sample}_variants.filtered.vcf.gz ${sample}_variants.filtered.chr2int.vcf.gz
...
#python OmicMind_wes_int2chr.v1.0.0.py ${sample}_vep.vcf ${sample}_vep.int2chr.vcf
...
bgzip ${sample}_variants.vep.vcf

bcftools index ${sample}_variants.vep.vcf.gz

# Convert back to original (1 -> chr1)
python OmicMind_wes_int2chr.v1.0.0.py ${sample}_variants.vep.vcf.gz ${sample}_variants.vep.int2chr.vcf.gz

gunzip ${sample}_variants.vep.int2chr.vcf.gz

bgzip ${sample}_variants.vep.int2chr.vcf

bcftools index ${sample}_variants.vep.int2chr.vcf.gz

rm ${sample}_variants.vep.vcf.gz*
...
bcftools annotate --threads 32 -a $CLINVAR_VCF \
	...
	-o ${sample}_variants.vep.clnvar.vcf.gz ${sample}_variants.vep.int2chr.vcf.gz

# Index the ClinVar annotated VCF
bcftools index ${sample}_variants.vep.clnvar.vcf.gz

rm ${sample}_variants.vep.int2chr.vcf.gz*
...
bcftools annotate --threads 32 \
	...
	-c INFO/gnomAD_AF \
	-O z \
	-o ${sample}_variants.vep.clnvar.gnomAD.bcf \
	${sample}_variants.vep.clnvar.vcf.gz

bcftools index ${sample}_variants.vep.clnvar.gnomAD.bcf

rm ${sample}_variants.vep.clnvar.vcf.gz*

bcftools view ${sample}_variants.vep.clnvar.gnomAD.bcf > ${sample}_variants.vep.clnvar.gnomAD.vcf

rm ${sample}_variants.vep.clnvar.gnomAD.bcf*
...
Intervar.py -b hg19 \
	-i ${sample}_variants.vep.clnvar.gnomAD.vcf --input_type=VCF \
	-o ${sample}_variants.vep.clnvar.gnomAD.intervar.vcf \
	-t $INTERVARDB \
	-d $HUMANDB
...
python bio_WES_add_chr_prefix_v2.py ${sample}_variants.vep.clnvar.gnomAD.intervar.vcf.hg19_multianno.txt.intervar ${sample}_variants.vep.clnvar.gnomAD.intervar.vcf.hg19_multianno.txt.chrPrefix.intervar

# Integrate ACGM infor into the annotated vcf
python bio_WES_integrate_ACGM_v2.py ${sample}_variants.vep.clnvar.gnomAD.vcf ${sample}_variants.vep.clnvar.gnomAD.intervar.vcf.hg19_multianno.txt.chrPrefix.intervar ${sample}_variants.vep.clnvar.gnomAD.intervar.vcf
