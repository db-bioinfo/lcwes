...
def convert_chr_names(input_file, output_file):
    """
    Convert chromosome names from '1' format to 'chr1' format in a VCF file.
    ...
    try:
        for line in infile:
            ...
            parts = line.split('\t')
            # Only add chr if it's not already there and it's a valid chromosome identifier
            if not parts[0].startswith('chr') and re.match(r'^([0-9]+|X|Y|M|MT)$', parts[0]):
                # Handle MT -> chrM conversion
                if parts[0] == 'MT':
                    parts[0] = 'chrM'
                else:
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python vcf_add_chr_prefix.py input.vcf[.gz] output.vcf[.gz]")
        sys.exit(1)
